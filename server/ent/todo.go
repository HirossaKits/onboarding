...
func (*Todo) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		switch columns[i] {
		case todo.FieldUserID, todo.FieldTitle, todo.FieldContent:
			values[i] = new(sql.NullString)
		case todo.FieldCreatedAt:
			values[i] = new(sql.NullTime)
		case todo.FieldID:
			values[i] = new(uuid.UUID)
		case todo.ForeignKeys[0]: // user_user_todo
	...
	for i := range columns {
		...
		case todo.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				return fmt.Errorf("unexpected type %T for field user_user_todo", values[i])
			} else if value.Valid {
				t.user_user_todo = new(uuid.UUID)
				*t.user_user_todo = *value.S.(*uuid.UUID)
			}
		}
	}
	return nil
